...
function pickGreeting(): { title: string; subtitle: string } {
  ...
  if (h >= 5 && h < 12)
    return { title: "Доброе утро ☀️", subtitle: "Кофе налит — пора зарабатывать." };
  if (h >= 12 && h < 17)
    return { title: "Привет 👋", subtitle: "День в разгаре." };
  if (h >= 17 && h < 22)
    return { title: "Добрый вечер 🌆", subtitle: "Закрываем сделки красиво." };
  return { title: "Поздний час 🌙", subtitle: "Тоже не спишь? Уважаем." };
}
...
export default function LoginPage() {
  ...
  async function onSubmit(e: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      if (remember) window.localStorage.setItem(REMEMBER_KEY, u);
      ...
      <div className="relative w-full max-w-md animate-fade-in-up">
        <Card className="p-8 lg:p-10 bg-white/85 backdrop-blur-md">
          <div className="mb-8">
            ...
            <p className="text-sm text-ink-soft mt-1.5">{greeting.subtitle}</p>
          </div>
          ...
          <form onSubmit={onSubmit} className="flex flex-col gap-4" autoComplete="on">
            ...
            <div className="flex items-center justify-between text-sm">
              ...
              <button
                ...
                onClick={() => alert("Обратитесь к администратору для сброса пароля.")}
              >
                Забыли пароль?
              </button>
            </div>
